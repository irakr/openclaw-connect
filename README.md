# OpenClaw HTTPS Front-End Setup

`openclaw-gateway-https.sh` automates the LAN-hardened gateway setup: HTTPS
termination via Caddy + mkcert, loopback-only gateway, cert rotation when your
DHCP lease changes, and clear onboarding steps for remote browsers (Mac, etc.).
...
./openclaw-gateway-https.sh -d walc-gw.local   # pick any hostname
```
...
4. **Host resolution** – Add an `/etc/hosts` entry on the Mac pointing your
   chosen hostname to the gateway’s current LAN IP (e.g., `192.168.1.48   walc-gw.local`).
5. **Open Control UI** – Visit `https://<hostname>/` in the browser. When the UI
   prompts for auth, paste your gateway token (e.g., `admin`).
